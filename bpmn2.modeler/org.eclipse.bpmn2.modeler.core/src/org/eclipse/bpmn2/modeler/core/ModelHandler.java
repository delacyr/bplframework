		...
		if (domain != null) {
			domain.getCommandStack().execute(new RecordingCommand(domain) {
				...
				protected void doExecute() {
					...
					VrProcess vrprocess = createVrProcess();
					vrprocess.setName(name+Messages.ModelHandler_VrProcess);
					// the Process ID should be the same as the resource name
					String filename = resource.getURI().lastSegment();
					if (filename.contains(".")) //$NON-NLS-1$
						filename = filename.split("\\.")[0]; //$NON-NLS-1$
					vrprocess.setId( ModelUtil.generateID(vrprocess,resource,filename) );

					// create StartEvent
					StartEvent startEvent = create(StartEvent.class);
//					startEvent.setName("Start Event");
					vrprocess.getFlowElements().add(startEvent);
					
					// create SequenceFlow
					SequenceFlow flow = create(SequenceFlow.class);
					vrprocess.getFlowElements().add(flow);
					
					// create EndEvent
					EndEvent endEvent = create(EndEvent.class);
//					endEvent.setName("End Event");
					vrprocess.getFlowElements().add(endEvent);
					...
					// add to BPMNDiagram
					plane.setBpmnElement(vrprocess);
					bpmnDiagram.setPlane(plane);
					bpmnDiagram.setName(name+Messages.ModelHandler_VrProcess_Diagram);
					getDefinitions().getDiagrams().add(bpmnDiagram);
				}
			});
		}
